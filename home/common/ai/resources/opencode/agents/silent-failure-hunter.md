---
description:
  Detects silent failures, swallowed exceptions, and missing error handling. Use
  PROACTIVELY during code review or QA.
...
**File**: `db/repository.py:145` **Issue**: Transaction failure silently ignored
**Impact**: Data may not persist, user unaware **Fix**: Re-raise or return error
status

### High: Ignored API Error Response

**File**: `services/payment.py:78` **Issue**: Never checks response.success
**Impact**: Failed payments marked as successful **Fix**: Check response and
handle failure case
