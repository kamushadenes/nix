...
Use the **task tool** with `subagent_type="git-committer"` for the full PR
workflow.

In your prompt to the agent, include:

1. **Task context**: Summarize what was being worked on in this session (from
   conversation history or todo list)
...
Once the PR is created, follow these steps:
...
Return the agent's summary (including PR URL) and the final check/mergeable
status to the user.
