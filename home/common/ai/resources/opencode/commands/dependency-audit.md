...
- Package files:
  !`find . -name "package.json" -o -name "requirements.txt" -o -name "go.mod" -o -name "Cargo.toml" -o -name "*.nix" 2>/dev/null | head -10`

## Your Task

Delegate the full dependency audit to the `dependency-checker` subagent to keep
verbose audit output out of main context.

Use the **task tool** with:

- `subagent_type`: "dependency-checker"
- `prompt`: Include the package files context above and request a comprehensive
  audit covering:
