...
- PR base:
  !`gh pr view --json baseRefName -q '.baseRefName' 2>/dev/null || echo "no PR"`
...
3. **Delegate to `pr-describer` agent:**
   ...
   > Update PR description. Current branch: [current_branch] Base branch:
   > [base_branch] PR title: [title]
