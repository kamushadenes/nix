...
Create a new NixOS LXC container on Proxmox with impermanence, configured for
...
- Extract specs from flags: `--ram`, `--cores`, `--disk`, `--vlan`,
  `--privileged`
- Skip Phases 2-5 (no plan mode, no interactive questions for specs)
- Still ask for hostname and storage (via the question tool) since these aren't
  known from origin
...
**This command operates in plan mode.** Gather all requirements before creating
anything.
...
- `services` - Comma-separated service types (e.g., `postgresql`,
  `nginx,docker`)
- `description` - Human-readable description in quotes

---

## PHASE 4: Gather Requirements via the Question Tool

**For migration mode:** Only ask 4.1 (hostname) and 4.3 (storage), skip the
rest.
...
Confirm with user before proceeding with LXC creation.

---

## PHASE 7: Create LXC

**For migration mode:** Jump here directly with passed specs. **For normal
invocation:** Proceed after plan approval.
...
Note: For `--unprivileged`, use `1` for unprivileged (secure) or `0` for
privileged.
...
After the container has obtained an IP via DHCP, update the PCT config to make
it static:
...
**Note:** The "Address already assigned" warning is expected since the container
already has this IP via DHCP. The important thing is that the config is now
static and will persist across reboots.
...
**For migration mode:** Stop here. Output the IP address and return to
/migrate-lxc:
...
Reference the service configuration table from migrate-lxc for each service
type.
...
**For migration mode:** Output only the new LXC IP address for /migrate-lxc to
continue:
