...
Fix all actionable feedback on a PR: human review conversations, standalone review comments, and code scanning (SAST) alerts.
...
Reviews can have top-level body text with action items not tied to specific threads. Fetch these too:
...
Filter out reviews with empty bodies — only keep those with substantive action items:
...
Fetch SAST alerts for the PR's head branch. This may return empty or 404 if code scanning is not enabled — that's fine, just skip SAST processing.
...
- Brief overview of feedback themes (e.g., "3 style issues, 2 logic concerns, 4 SAST warnings")

If zero items across all categories, inform the user and stop.

### Step 4: Triage All Feedback

Batch-evaluate all feedback (threads + standalone comments + SAST alerts) using the suggestion-critic Task:
...
Group all actionable items (VALID threads, ACTIONABLE SAST alerts) by file path. Process files in alphabetical order. Within each file, process from **bottom to top** (highest line number first) to avoid line number shifts.
...
1. **VALID**: Read the file, fix the code, reply explaining what was changed, track thread ID for resolution
...
After fixing all issues, use `Skill(skill="commit")` with a message summarizing all fixes (review feedback + SAST).
...
Summarize:
- **Fixed** (N): threads/alerts where code was changed (file/line, brief description)
- **Dismissed** (M): threads replied to as invalid + SAST alerts dismissed (with reason)
