...
- **Global scope**: `/private$HOME/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/private$HOME/.claude/notes/`)
...
2. Glob `/private$HOME/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/private$HOME/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/private$HOME/.claude/notes/` — works in any directory
