...
### 3. Trace with Multi-Model Assistance

Get help tracing the execution flow:

```python
# Claude: Full flow analysis
claude_trace = clink(
    prompt="""Trace the execution flow for POST /api/orders:
...
Files to examine: src/api/orders.py, src/services/order.py, src/models/order.py""",
    cli="claude",
    files=["src/api/orders.py", "src/services/order.py", "src/models/order.py"]
)

# Codex: Focus on specific concern
codex_trace = clink(
    prompt="""Trace specifically where validation happens in the order creation flow.
