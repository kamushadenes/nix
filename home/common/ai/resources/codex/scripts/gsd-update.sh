...
NIX_CONFIG_GSD="${HOME}/.config/nix/config/home/common/ai/resources/codex/gsd"
...
cd "$tmpdir"
npm exec --yes -- get-shit-done-cc --codex --local 2>&1

echo ""
echo "Syncing files to nix config..."

# Full sync of all categories (--delete removes files dropped by upstream)
rsync -a --delete "$tmpdir/.codex/agents/" "$NIX_CONFIG_GSD/agents/"
rsync -a --delete "$tmpdir/.codex/skills/" "$NIX_CONFIG_GSD/skills/"
rsync -a --delete "$tmpdir/.codex/get-shit-done/" "$NIX_CONFIG_GSD/get-shit-done/"

# Top-level metadata files
cp "$tmpdir/.codex/gsd-file-manifest.json" "$NIX_CONFIG_GSD/gsd-file-manifest.json"
cp "$tmpdir/.codex/config.toml" "$NIX_CONFIG_GSD/config.toml" 2>/dev/null || true
...
find "$NIX_CONFIG_GSD" \( -name "*.md" -o -name "*.js" -o -name "*.json" -o -name "*.cjs" -o -name "*.sh" -o -name "*.toml" \) \
	-exec sed -i '' "s|${tmpdir}/.codex/|\$HOME/.codex/|g" {} +
