...
- **Global scope**: `/private$HOME/.codex/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/private$HOME/.codex/notes/`)
...
2. Glob `/private$HOME/.codex/notes/*.md` (if directory exists) — global notes
...
Global (/private$HOME/.codex/notes/):
...
2. **No `.planning/`**: Falls back to global `/private$HOME/.codex/notes/` — works in any directory
