...
```bash
PHASE_INFO=$(node "/private$HOME/.codex/get-shit-done/bin/gsd-tools.cjs" find-phase "${PHASE}")
```
...
```bash
PHASE_CHECK=$(node "/private$HOME/.codex/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}" --pick found)
...
```bash
PHASE_DIR=$(node "/private$HOME/.codex/get-shit-done/bin/gsd-tools.cjs" find-phase "${PHASE}" --raw)
```
