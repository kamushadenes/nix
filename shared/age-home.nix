# Patched version of agenix home-manager module (ryantm/agenix modules/age-home.nix)
# Fix: clean up stale generation directories before creating new ones to prevent
# infinite crash loops when the mount secrets script fails mid-decryption.
# Upstream issue: if decryption fails, read-only .tmp files are left behind.
# On retry, the same generation number is computed, and writing to the existing
# directory fails with "permission denied", causing a launchd KeepAlive crash loop.
{
  config,
  options,
  lib,
...
let
  ...
  newGeneration = ''
    ...
    # Clean up stale generation from a previous failed run to prevent
    # permission denied errors on read-only .tmp files left behind
    if [ -d "${cfg.secretsMountPoint}/$_agenix_generation" ]; then
      echo "[agenix] cleaning up stale generation $_agenix_generation..."
      chmod -R u+w "${cfg.secretsMountPoint}/$_agenix_generation" 2>/dev/null || true
