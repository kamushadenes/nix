...
_c_default() {
    ...
    if test -n "${TMUX:-}"; then
        ...
        if [ -n "${CLAUDE_CONFIG_DIR:-}" ]; then
            ...
            # Remove from tmux global env (prevents inheritance from previous -e usage)
            # then start session without the variable
            tmux set-environment -gu CLAUDE_CONFIG_DIR 2>/dev/null || true
