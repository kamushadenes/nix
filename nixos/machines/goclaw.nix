...
exec /app/data/.runtime/gam/gam "$@"'
  ...
  # granola-cli install. Bun-compiled binary; upstream releases ship only
  # macOS/glibc-Linux variants, so this fetches the musl rebuild from the
  # kamushadenes/granola-cli fork (same source, rebuilt with
  # `bun build --target=bun-linux-x64-musl`). Real binary lives at
  # /app/data/.runtime/granola/granola; a thin POSIX-sh wrapper at
  # /app/data/.runtime/bin/granola is the dashboard-facing entrypoint
  # that:
  #   1. Prepends /app/data/.runtime/npm-global/bin so granola can spawn
  #      mcporter (the dashboard's "Node Packages" install lands the
  #      mcporter shim there but does not extend the goclaw user's
  #      default PATH, which is just /usr/bin etc.).
  #   2. In sandboxes (HOME=/tmp, /app/data is read-only), copies the
  #      materialized OAuth + config dirs from
  #      /app/data/.runtime/granola-cli/{mcporter,granola-cli}/ onto
  #      tmpfs HOME so mcporter's credentials.json + granola's
  #      mcporter.json resolve in the writable per-spawn dir. In main
  #      (HOME=/app), the bind-mount already places those dirs at
  #      /app/.mcporter and /app/.config/granola-cli, so the wrapper
  #      shortcuts and just execs the real binary.
  # Idempotent on the pinned binary sha and on the wrapper sha.
  systemd.services.goclaw-granola-bin = {
    ...
    script = ''
      ...
      real_dir=${goclaw-data-vol}/.runtime/granola
      real=$real_dir/granola
      wrap=${goclaw-data-vol}/.runtime/bin/granola
      ...
      mkdir -p "$real_dir"
      chown 1000:1000 "$real_dir"

      current_sha=""
      [ -f "$real" ] && current_sha=$(sha256sum "$real" | cut -d' ' -f1)
      if [ "$current_sha" != "$target_sha" ]; then
        ...
        install -m 0755 -o 1000 -g 1000 "$tmp/granola" "$real"
        echo "granola installed at $real"
      else
        echo "granola already at pinned musl build ($target_sha)"
      fi

      # Drop legacy install at bin/granola if it's the raw 100MB binary
      # (sha matches target_sha but is NOT the wrapper). The wrapper is
      # smaller and has a different sha; if the bin path holds the real
      # binary, replace it below.
      wrap_content='#!/bin/sh
# granola-cli wrapper. Generated by goclaw-granola-bin.service.
# Augments PATH for mcporter resolution and stages OAuth state into
# the writable sandbox HOME on each spawn.
export PATH="/app/data/.runtime/npm-global/bin:''${PATH:-/usr/bin:/bin}"
if [ "''${HOME:-}" = "/tmp" ] && [ -d /app/data/.runtime/granola-cli ]; then
  mkdir -p "$HOME/.mcporter" "$HOME/.config/granola-cli"
  if [ -d /app/data/.runtime/granola-cli/mcporter ]; then
    cp -rf /app/data/.runtime/granola-cli/mcporter/. "$HOME/.mcporter/" 2>/dev/null || true
    chmod 0700 "$HOME/.mcporter" 2>/dev/null || true
    find "$HOME/.mcporter" -type f -exec chmod 0600 {} + 2>/dev/null || true
  fi
  if [ -d /app/data/.runtime/granola-cli/granola-cli ]; then
    cp -rf /app/data/.runtime/granola-cli/granola-cli/. "$HOME/.config/granola-cli/" 2>/dev/null || true
  fi
fi
exec /app/data/.runtime/granola/granola "$@"'
      desired_wrap_sha=$(printf '%s\n' "$wrap_content" | sha256sum | cut -d' ' -f1)
      current_wrap_sha=""
      [ -f "$wrap" ] && current_wrap_sha=$(sha256sum "$wrap" | cut -d' ' -f1)
      if [ "$desired_wrap_sha" != "$current_wrap_sha" ]; then
        printf '%s\n' "$wrap_content" > "$wrap.new"
        chmod 0755 "$wrap.new"
        chown 1000:1000 "$wrap.new"
        mv "$wrap.new" "$wrap"
        echo "granola wrapper updated"
      else
        echo "granola wrapper already up to date"
      fi
    '';
  };

  # Sandbox-side creds materializer. Mirrors the host bind-mount source
  # (/var/lib/goclaw-granola-creds/{mcporter,granola-cli}) into the
  # shared `app_goclaw-data` volume at /app/data/.runtime/granola-cli/
  # so the read-only sandbox mount can see the OAuth refresh tokens
  # and granola's mcporter.json. Triggered on boot via wantedBy and on
  # any change under the host source by the matching systemd.path unit
  # below.
  systemd.services.goclaw-granola-creds-sync = {
    description = "Mirror granola-cli + mcporter creds into goclaw shared volume";
    after = [ "goclaw.service" ];
    wants = [ "goclaw.service" ];
    wantedBy = [ "multi-user.target" ];
    ...
    script = ''
      ...
      src=${goclaw-granola-creds-host}
      dest=${goclaw-data-vol}/.runtime/granola-cli
      ...
      install -d -m 0750 -o 1000 -g 1000 "$dest/mcporter"
      install -d -m 0750 -o 1000 -g 1000 "$dest/granola-cli"

      if [ -d "$src/mcporter" ]; then
        rsync -a --delete \
          --chown=1000:1000 \
          --chmod=D0750,F0600 \
          "$src/mcporter/" "$dest/mcporter/"
      fi
      if [ -d "$src/granola-cli" ]; then
        rsync -a --delete \
          --chown=1000:1000 \
          --chmod=D0750,F0600 \
          "$src/granola-cli/" "$dest/granola-cli/"
      fi
      echo "granola creds mirrored to $dest"
    '';
  };

  # Trigger goclaw-granola-creds-sync whenever mcporter rotates a token
  # or granola rewrites its config. systemd.path watches both the
  # mcporter and granola-cli subdirs (PathChanged fires on close-after-
  # write and on metadata change, which covers the rename-into-place
  # writes mcporter does for credentials.json).
  systemd.paths.goclaw-granola-creds-sync = {
    description = "Watch granola-cli + mcporter creds for changes";
    wantedBy = [ "multi-user.target" ];
    pathConfig = {
      PathChanged = [
        "${goclaw-granola-creds-host}/mcporter"
        "${goclaw-granola-creds-host}/granola-cli"
      ];
      Unit = "goclaw-granola-creds-sync.service";
    };
  };
